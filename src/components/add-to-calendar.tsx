"use client";

import { Calendar, Download, ExternalLink } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/ui/tooltip";
import { 
  createCalendarEvent, 
  generateGoogleCalendarUrl, 
  generateIcsDataUrl,
  CalendarEvent
} from "@/lib/utils";
import { CarBootSale } from "@/types";
import { useState, useEffect } from "react";

interface AddToCalendarProps {
  sale: CarBootSale;
  className?: string;
}

export function AddToCalendar({ sale, className = "" }: AddToCalendarProps) {
  const [downloadUrls, setDownloadUrls] = useState<{[key: string]: string}>({});

  useEffect(() => {
    return () => {
      Object.values(downloadUrls).forEach(url => {
        if (url.startsWith('blob:')) {
          URL.revokeObjectURL(url);
        }
      });
    };
  }, [downloadUrls]);

  const calendarEvent: CalendarEvent = createCalendarEvent(
    {
      name: sale.name,
      location: sale.location,
      address: sale.address || undefined,
      daysOfWeek: sale.daysOfWeek || [],
      startTime: sale.startTime,
      endTime: sale.endTime,
      carPrice: sale.carPrice,
      vanPrice: sale.vanPrice,
      organiserEmail: sale.organiserEmail || '',
      organiserPhone: sale.organiserPhone || '',
      otherInfo: sale.otherInfo || undefined,
      what3words: sale.what3words || undefined,
    },
    typeof window !== 'undefined' ? window.location.href : undefined
  );

  const handleGoogleCalendar = () => {
    const googleUrl = generateGoogleCalendarUrl(calendarEvent);
    window.open(googleUrl, '_blank', 'noopener,noreferrer');
  };

  const handleIcsDownload = (calendarType: 'apple' | 'outlook') => {
    try {
      if (downloadUrls[calendarType] && downloadUrls[calendarType].startsWith('blob:')) {
        URL.revokeObjectURL(downloadUrls[calendarType]);
      }

      const dataUrl = generateIcsDataUrl(calendarEvent);
      setDownloadUrls(prev => ({ ...prev, [calendarType]: dataUrl }));

      const link = document.createElement('a');
      link.href = dataUrl;
      link.download = `${sale.name.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_car_boot_sale.ics`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    } catch (error) {
      console.error(`Error generating ${calendarType} calendar file:`, error);
    }
  };

  return (
    <TooltipProvider>
      <div className={`flex items-center gap-3 ${className}`}>
        <div className="flex items-center gap-2 text-sm text-gray-600">
          <Calendar className="h-4 w-4 text-primary" />
          <span className="font-medium">Add to Calendar:</span>
        </div>
        
        <div className="flex items-center gap-2">
          {/* Google Calendar */}
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                onClick={handleGoogleCalendar}
                variant="outline"
                size="sm"
                className="h-8 w-8 p-0 hover:bg-blue-50 hover:border-blue-300"
                data-testid="button-google-calendar"
              >
                <svg viewBox="0 0 24 24" className="h-4 w-4" fill="currentColor">
                  <path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10zm-1-6v-4h2v4h-2zm0-6V8h2v2h-2z" opacity="0.3"/>
                  <path d="M19 4h-1V2h-2v2H8V2H6v2H5c-1.11 0-1.99.9-1.99 2L3 20c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 16H5V9h14v11zM9 11H7v2h2v-2zm4 0h-2v2h2v-2zm4 0h-2v2h2v-2zm-8 4H7v2h2v-2zm4 0h-2v2h2v-2zm4 0h-2v2h2v-2z"/>
                </svg>
              </Button>
            </TooltipTrigger>
            <TooltipContent>
              <p>Add to Google Calendar</p>
            </TooltipContent>
          </Tooltip>

          {/* Apple Calendar */}
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                onClick={() => handleIcsDownload('apple')}
                variant="outline"
                size="sm"
                className="h-8 w-8 p-0 hover:bg-gray-100 hover:border-gray-300"
                data-testid="button-apple-calendar"
              >
                <svg viewBox="0 0 24 24" className="h-4 w-4" fill="currentColor">
                  <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/>
                </svg>
              </Button>
            </TooltipTrigger>
            <TooltipContent>
              <p>Download for Apple Calendar</p>
            </TooltipContent>
          </Tooltip>

          {/* Outlook Calendar */}
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                onClick={() => handleIcsDownload('outlook')}
                variant="outline"
                size="sm"
                className="h-8 w-8 p-0 hover:bg-blue-50 hover:border-blue-300"
                data-testid="button-outlook-calendar"
              >
                <svg viewBox="0 0 24 24" className="h-4 w-4" fill="currentColor">
                  <path d="M7.88 12.04q0 .45-.11.87-.1.41-.33.74-.22.33-.58.52-.37.2-.87.2t-.85-.2q-.35-.21-.57-.55-.22-.33-.33-.75-.1-.42-.1-.86t.1-.87q.1-.43.34-.76.22-.34.59-.54.36-.2.87-.2t.86.2q.35.21.57.55.22.34.31.77.1.43.1.88zM24 12v9.38q0 .46-.33.8-.33.32-.8.32H7.13q-.46 0-.8-.33-.32-.33-.32-.8V18H1q-.41 0-.7-.3-.3-.29-.3-.7V7q0-.41.3-.7Q.58 6 1 6h6.5V2.55q0-.44.3-.75.3-.3.75-.3h12.9q.44 0 .75.3.3.3.3.75V12zm-6-8.25v3h3v-3zm0 4.5v3h3v-3zm0 4.5v1.83l3.05-1.83zm-5.25-9v3h3.75v-3zm0 4.5v3h3.75v-3zm0 4.5v2.03l2.41 1.5 1.34-.8v-2.73zM9 3.75V6h2l.13.01.12.04v-2.3zM5.98 15.98q.9 0 1.6-.3.7-.32 1.19-.86.48-.55.73-1.28.25-.74.25-1.61 0-.83-.25-1.55-.24-.71-.71-1.24t-1.15-.83q-.68-.3-1.55-.3-.92 0-1.64.3-.71.3-1.2.85-.5.54-.75 1.3-.25.74-.25 1.63 0 .85.26 1.56.26.72.74 1.23.48.52 1.17.81.69.3 1.56.3zM7.5 21h12.39L12 16.08V17q0 .41-.3.7-.29.3-.7.3H7.5zm15-.13v-7.24l-5.9 3.54Z"/>
                </svg>
              </Button>
            </TooltipTrigger>
            <TooltipContent>
              <p>Download for Outlook</p>
            </TooltipContent>
          </Tooltip>
        </div>
      </div>
    </TooltipProvider>
  );
}
